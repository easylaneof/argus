...
@Component
@Slf4j
public class UntrackCommandProcessor extends AbstractCommandProcessor<SendMessage, SendResponse> {
    public UntrackCommandProcessor(HelpCommandProcessor helpCommandProcessor, CommandParser commandParser, LinkService linkService) {
    ...
    public SendMessage process(Update update) {
        ...
        if (result.isPresent()) {
            ...
            return send(update, "Untracked link %s".formatted(link));
        } else {
            logFailedLink(update);

            return send(update, "Failed untracking link %s".formatted(link));
        }
    }
