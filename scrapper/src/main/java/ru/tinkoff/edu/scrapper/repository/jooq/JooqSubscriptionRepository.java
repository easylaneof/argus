...
import static ru.tinkoff.edu.scrapper.entity.jooq.Tables.LINK;
import static ru.tinkoff.edu.scrapper.entity.jooq.Tables.SUBSCRIPTION;
...
public class JooqSubscriptionRepository implements SubscriptionRepository {
    ...
    public void deleteLinkFromChat(long chatId, Link link) {
        ...
        int updatedRows = create
            .delete(SUBSCRIPTION)
            .where(SUBSCRIPTION.CHAT_ID.eq(chat.getId()).and(SUBSCRIPTION.LINK_ID.eq(foundLink.getId())))
            .execute();
